...
# WH_KEYBOARD_LL 저수준 키보드 훅 (KbBlocker 타입 정의).
# 실행은 호출자가 stdin 으로 [KbBlocker]::Run([uint32]<pid>) 를 덧붙여 시작한다.
# 포그라운드 창의 PID 가 우리 앱(targetPid)일 때만 키 이벤트를 삼킨다(return 1).
# 그 외에는 CallNextHookEx 로 통과시킨다. 감지는 Electron 쪽 uiohook 이 담당한다.
